...
set "VCToolsInstallDir=C:\fake\vc\tools\"
set "UniversalCRTSdkDir=C:\fake\ucrt\"
...
'
    ...
    Assert-True (($selection.actions.PSObject.Properties.Name -join ",") -eq "npm_ci,npm_build,cargo_release_build,vpk_pack,cargo_deny_advisories,native_smoke") "unsigned emits exact action set"
    Assert-True ($selection.actions.npm_ci.program -eq (Join-Path $FakeBin "npm.cmd")) "npm action uses selected npm"
    ...
    Assert-True ($null -eq $selection.actions.smctl_sign) "unsigned omits smctl action"
    Assert-True ($null -eq $selection.actions.signtool_verify) "unsigned omits SignTool action"
    Assert-True ($selection.msvc_environment.VCToolsVersion -eq "14.44.35207") "selection captures activated MSVC toolset"
    Assert-True ($selection.msvc_environment.WindowsSDKVersion -eq "10.0.26100.0\") "selection captures activated Windows SDK version"
    Assert-True (($selection.msvc_environment.PSObject.Properties.Name -join ",") -eq "PATH,INCLUDE,LIB,LIBPATH,VCINSTALLDIR,VCToolsInstallDir,VCToolsVersion,UniversalCRTSdkDir,UCRTVersion,WindowsSdkDir,WindowsSdkBinPath,WindowsLibPath,WindowsSDKVersion") "selection emits exact MSVC environment allowlist"
    ...
    Assert-True ($selection.actions.signing_auth_preflight.program -eq $PowerShellPath) "signed auth action uses selected PowerShell"
    Assert-True ($selection.actions.smctl_sign.program -eq (Join-Path $FakeBin "smctl.cmd")) "signed action uses selected smctl"
    Assert-True ($selection.actions.signtool_verify.program -eq $signtoolPath) "signed action uses selected SignTool"
